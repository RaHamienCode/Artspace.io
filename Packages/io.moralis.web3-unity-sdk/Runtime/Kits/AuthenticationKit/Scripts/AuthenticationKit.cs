...
namespace MoralisUnity.Kits.AuthenticationKit
{
    ...
    public class AuthenticationKit : MonoBehaviour
    {
        ...
        public async void ConnectWallet_WithButton(){
            Debug.Log("Loggin in with web 3");
            await LoginWithWeb3();
        }
      ...
      /*  public void GetUserAddress()
        {
            try{
            address = Web3GL.Account().ToLower();
            } catch (Exception e){
                Debug.Log("We tried")
            }
        }
    */  
        ...
        public async void WalletConnect_OnConnectedEventSession(WCSessionData wcSessionData)
        {
            ...
            address = wcSessionData.accounts[0].ToLower();
            string appId = Moralis.DappId;
            long serverTime = 0;
